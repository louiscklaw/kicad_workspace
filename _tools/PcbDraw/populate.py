...
PCBDRAW_BIN = os.getenv('PCBDRAW_BIN',os.path.join(CWD, 'pcbdraw.py'))
    ...
    for item in content:
        ...
        for x in item["steps"]:
            if x["comment"].find('[x]') >= 0 or x["comment"].find('[ ]') >= 0:
                ...
                print('generate on comment:%s ' % x['comment'])
                counter += 1
...
if __name__ == "__main__":
    ...
    pprint(content)
    print('before --- ')
    content = generate_images(content, args["board"], args["libs"], args["params"], args["img_name"], args["output"])
